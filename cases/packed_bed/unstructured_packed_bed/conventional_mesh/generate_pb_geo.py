...
#Length of cells
h_min = 0.00005; h_max = 0.0005 # Minimal and maximal lengths of the tetrahedral cells

i = 0
spheres = ""
for row in x:
	spheres += 'Sphere(' + str(int(i+2)) + ') = {' + str(x[i]) + ', ' + str(y[i]) + ', ' + str(z[i]) + ', ' + str(r[i]) + '};\n'
	i += 1

f_template = open(geo_template, 'r')
geo_text = f_template.read()
template = Template(geo_text)
text = template.render(x0 = x0, y0 = y0, z0 = z0, dx = dx, dy = dy, dz = dz, spheres = spheres, pb_index = i+2, last_sphere_volume = i+1, last_sphere_surface = i+6, h_min = h_min, h_max = h_max)
