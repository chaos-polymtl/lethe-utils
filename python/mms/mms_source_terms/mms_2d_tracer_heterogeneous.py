...
from sympy import symbols, sin, cos, pi, diff, tanh
...
k_in = pi**2  # Reaction coefficient inside the particle
k_out = 0     # Reaction coefficient outside the particle
t = 0.1       # Interface thickness
order = 1.5   # Reaction order
...
k = k_in + (k_out - k_in) * (0.5 + 0.5 * tanh(sdf / t))
...
# Tracer MMS Solution
T = (0.5 + 0.5 * tanh(sdf / t)) * (1 + sin(a*x) * cos(a*y)) / 2
