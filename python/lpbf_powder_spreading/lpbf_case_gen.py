...
# List of all the diameters we want to use
...
Restant_frequency = insert_frequency * 20
...
time_per_layer = (domain_lenght + blade_thickness) / blade_speed
...
while solid_obj_id - 4 < number_of_layers:
    ...
    coater_param = (f"   subsection solid object {str(solid_obj_id)} \n"
                    ...
                    f"         set simplex             = true\n"
                    f"         set initial refinement  = 0\n"
                    f"         set initial translation = -0.0005, {delta_b_p}, 0\n"
                    f"      end\n"
                    f"      subsection translational velocity\n"
                    f"         set Function expression = {coater_function}; 0; 0\n"
                    f"      end\n"
...
t1 = every_starting_time[0] + (0.01 + blade_thickness) * 1.08 / blade_speed
t2 = t1 + plate_displacement_time * 3.

# 0.0241 is the initial translational displacement of the second separator.
t3 = every_starting_time[0] + (0.0241 + blade_thickness) * 1.05 / blade_speed
...
for i in range(1, number_of_layers):
    # HARD CODED
    t1 = every_starting_time[i] + (0.01 + blade_thickness) * 1.08 / blade_speed
    t2 = t1 + plate_displacement_time * 1.  # The reservoir moves...

    # HARD CODED
    t3 = every_starting_time[i] + (0.0241 + blade_thickness) * 1.05 / blade_speed
    t4 = t3 + plate_displacement_time * 1.
...
# Custom_diameters = ,                                 Custom_volume_fractions = ,
