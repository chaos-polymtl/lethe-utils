...
for i in range(len(prm_file_names)):
    ...
    number_of_layers = int((lines[2]).split('=')[1]) + 2
    blade_speed = float((lines[3]).split('=')[1])
    delta_B_P = float((lines[4]).split('=')[1])
    delta_o = float((lines[5]).split('=')[1])
    delta_n = float((lines[6]).split('=')[1])
    GAP = float((lines[8]).split('=')[1])
    first_layer_extrusion = float((lines[9]).split('=')[1])
    n_layer_extrusion = float((lines[10]).split('=')[1])
    ...
    measuring_time_PB_rel_density = every_starting_time + 0.85 * blade_time_per_layer
    ...
    BP_rel_density_each_layer = np.zeros(number_of_layers)
    BP_rel_density_cumulative = np.zeros(number_of_layers)
    volume_on_BP = np.zeros(number_of_layers)
    volume_on_feeder = np.zeros(number_of_layers)
    feeder_rel_density = np.zeros(number_of_layers)
    ...
    for index, k in enumerate(vtu_measure_BP_rel_density[1:], start=1):
